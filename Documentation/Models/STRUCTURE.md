...
Each property is defined as an object in the `properties` array:
...
```d3e
{
    ...
    // Optional: defaultValue `expression`
    // Optional: longText true
    // Optional: collection true
    // Optional: child true
    // Optional: inverse true
    // Optional: inverseProperty propertyName
    // Optional: existsIf `expression`
    // Optional: computed true
    // Optional: computation `expression`
    // Optional: optionset OptionSetName
    // Optional: referenceFrom propertyName
    // Optional: transient true
}
```

**Common Property Attributes:**
- `name`: The property's display name.
- `type`: Data type (String, Integer, Boolean, Date, DateTime, Double, OptionSet, another Model, etc.).
- `required true`: Property must have a value.
- `unique true`: Value must be unique across all records.
- `defaultValue`: Sets a default value (e.g., `defaultValue true`, `defaultValue now`).
- `longText true`: For large text fields.
- `collection true`: Indicates a list/array of values.
- `child true`: Indicates a child relationship (lifecycle tied to parent).
- `inverse true` and `inverseProperty`: For bidirectional relationships.
- `existsIf`: Conditional existence (property shown only if condition is met).
- `computed true` and `computation`: Computed property, not stored.
- `optionset`: Restricts value to a predefined set (enum-like).
- `referenceFrom`: For intra-model references.
- `transient true`: Not persisted in the database.

---

## 3. Model-Level Attributes

- `parent ParentModel`: Inheritance; this model extends another.
- `master MasterModel`: Master-child relationship; this model is a child of another.
- `singleton true`: Only one instance exists.
- `creatable false`: Cannot be created directly (used as a component/child).
- `embedded true`: Always present as part of a parent.
- `transient true`: Not persisted; used for temporary/calculated data.
- `display propertyName`: Which property to use as the display label.
- `needCreatedDate true`: Adds a created date timestamp.
- `needUpdatedDate true`: Adds an updated date timestamp.

---

## 4. Actions Block

Models can define actions (methods, triggers, or lifecycle hooks):

```d3e
actions [
    {
        name 'ActionName'
        // Optional: runOn OnCreate/OnUpdate/OnDelete
        block ```
            // D3E code to execute
        ```
    }
...
Model {
    ...
    properties [
        ...
        }
        {
            ...
            optionset StatusOptionSet
            defaultValue `'StatusOptionSet.Active'`
        }
        {
            name 'Related Items'
            type RelatedModel
...
## 6. Special Model Types

- **Struct:** Used for non-persistent, utility objects (e.g., search criteria).
- **OptionSet:** Enum-like, for dropdowns and restricted values.
...
- **Master-Child:** `master`
- **Inverse/Bidirectional:** `inverse true`, `inverseProperty`
- **Collections:** `collection true`
- **Embedded:** `embedded true` or `child true` in property

---

## 8. Timestamps

- `needCreatedDate true`: Adds a created date field (auto-managed).
- `needUpdatedDate true`: Adds an updated date field (auto-managed).

---

## 9. Display

- `display propertyName`: Sets the default property to show in UI lists.

---

## 10. OptionSet Example
...
## 12. JSON Template for New Models
